...
class URLBruteforcer():
    ...
    def __init__(self, host: str, word_dictionary: WordDictonary, nb_thread: int = MAX_NUMBER_REQUEST, 
                 status_code: list = VALID_STATUS_CODE, proxy: dict = proxy_default_dict, directories_to_ignore: list = []):
        self.host = host
    ...
    def send_requests_with_all_words(self, url: str = None) -> None:
        ...
        url_completed = [ urljoin(url, word) for word in self.word_dictionary if word != '/' and word != '' ]
    ...
    def _remove_invalid_url_from_directory_found(self, directories_found: list, url: str) -> list:
        return [ dir_to_test for dir_to_test in directories_found if dir_to_test is not None and dir_to_test != url ]
...
def number_of_thread(value) -> int:
    ...
    if value > URLBruteforcer.MAX_NUMBER_REQUEST:
        raise argparse.ArgumentError('The number of thread is to high. Current: {}, Max: {}'.format(value, URLBruteforcer.MAX_NUMBER_REQUEST))
    return value
